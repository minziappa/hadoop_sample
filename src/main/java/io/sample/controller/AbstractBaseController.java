...
public abstract class AbstractBaseController {
	...
	public void handleLogin(HttpSession session, AdminService adminService) throws Exception {

		Object obj = SecurityContextHolder.getContext().getAuthentication().getPrincipal();

		if(obj instanceof ExtendUser) {
			ExtendUser extendUser = (ExtendUser) obj;
			logger.info("index - game");
			session.setAttribute("admin", extendUser.getHadoopAdmin());
			HadoopGameModel apnsGame = adminService.selectGame(extendUser.getHadoopAdmin().getGameId());
			session.setAttribute("game", apnsGame);
			session.setAttribute("adminName", extendUser.getHadoopAdmin().getAdminName());
